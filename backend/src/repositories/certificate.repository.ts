...
export function insert(record) {
  prepare(
    'INSERT INTO certificate_issuances (id, person_id, certificate_type, source_id, register_book, register_page, register_entry, person_full_name, issued_at, created_at)' +
      ' VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  ).run(
    record.id,
    record.personId,
...
export function findMany(filter: any = {}) {
  ...
  return prepare(
    'SELECT id, person_id AS personId, certificate_type AS type, register_book AS registerBook,' +
      ' register_page AS registerPage, register_entry AS registerEntry, person_full_name AS personFullName,' +
