...
import * as familyRepository from '#/repositories/family.repository.js'
import * as familyMemberRepository from '#/repositories/family-member.repository.js'
import * as personRepository from '#/repositories/person.repository.js'
import { runInTransaction } from '#/repositories/query-helpers.js'
...
function assertHeadAvailable(familyId, relationship, exceptId) {
 ...
 */
export function openMembership(input, timestamp, options = {}) {
 ...
 */
export function move({ personId, toFamilyId, relationship, moveDate }) {
  const timestamp = now()
