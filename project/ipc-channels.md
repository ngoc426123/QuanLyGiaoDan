...
| ---------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `dashboard:getSummary` | `{ month?: 'YYYY-MM' }` | Tổng số giáo dân còn sống, hộ, giáo họ, phân bố, báo cáo mục vụ tháng và các danh sách chất lượng dữ liệu | Tháng mặc định là tháng hiện tại; gom bằng truy vấn SQL, không truy vấn lặp theo từng hộ |
...
| Kênh                | Payload vào                                                                                                                                                                                                  | Dữ liệu trả về                                   | Ghi chú                                                                                        |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| `person:list`       | `{ zoneId?, familyId?, withoutFamily?, gender?, isAlive?, search?, page, pageSize, sortBy, sortDir }`                                                                                                        | `Person[]` kèm `familyName`, `zoneName` + `meta` | `search` so khớp trên `full_name_ascii`; `withoutFamily` chỉ lấy người chưa thuộc hộ hiện hành |
| `person:getById`    | `{ id }`                                                                                                                                                                                                     | `Person` kèm hộ hiện hành + lịch sử hộ           | `NOT_FOUND` nếu không tồn tại                                                                  |
| `person:create`     | `{ fullName, givenName?, holyName?, gender?, birthDate?, deathDate?, phone?, email?, occupation?, secondaryPhone?, residenceStatus?, pastoralStatus?, pastoralNote?, source?, sacraments?, note?, family? }` | `Person` vừa tạo                                 | Xem ghi chú dưới bảng                                                                          |
